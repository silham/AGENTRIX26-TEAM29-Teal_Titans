...
def get_case(db: Session, *, case_id: UUID, user_id: str) -> Case | None:
    return db.scalar(
        select(Case)
...
def delete_case(db: Session, *, case_id: UUID, user_id: str) -> bool:
    case = db.scalar(select(Case).where(Case.id == case_id, Case.user_id == user_id))
    if case is None:
        return False
    db.delete(case)
    db.commit()
